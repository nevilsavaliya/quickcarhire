...
//
// This file is part of pdf software library.
//
// pdf is free software: you can redistribute it and/or modify it
// under the terms of the GNU Lesser General Public License as
// published by the Free Software Foundation, either version 3 of the
// License, or (at your option) any later version.
//
// pdf is distributed in the hope that it will be useful, but
// WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
// See the GNU Lesser General Public License for more details.
//
// You should have received a copy of the License
// along with pdf. If not, see
 ...
 * pdf project (http://www.tcpdf.org) was originally derived in 2002 from the Public Domain FPDF class by Olivier Plathey (http://www.fpdf.org), but now is almost entirely rewritten.<br>
 * <h3>pdf main features are:</h3>
 * <ul>
 ...
 * pdf project (http://www.tcpdf.org) has been originally derived in 2002 from the Public Domain FPDF class by Olivier Plathey (http://www.fpdf.org), but now is almost entirely rewritten.<br>
 * @package com.tecnick.tcpdf
...
class TCPDF {
	...
	public function __construct($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false, $pdfa=false) {
		...
		$this->file_id = md5(TCPDF_STATIC::getRandomSeed('pdf'.$orientation.$unit.$serformat.$encoding));
	...
	public function Error($msg) {
		...
		if (defined('K_TCPDF_THROW_EXCEPTION_ERROR') AND !K_TCPDF_THROW_EXCEPTION_ERROR) {
			die('<strong>pdf ERROR: </strong>'.$msg);
		} else {
			throw new Exception('pdf ERROR: '.$msg);
		}
	}
	...
	protected function ImagePngAlpha($file, $x, $y, $wpx, $hpx, $w, $h, $type, $link, $align, $resize, $dpi, $palign, $filehash='') {
		...
		if ($parsed === false) {
			if (empty($parse_error)) {
				$this->Error('pdf requires the Imagick or GD extension to handle PNG images with alpha channel.');
			} else {
	...
	protected function _putcatalog() {
		...
		if (!empty($this->pdflayers)) {
			...
			$out .= ' /Creator '.$this->_textstring('pdf', $oid);
			$out .= ' /BaseState /ON';
			$out .= ' /OFF ['.$lyrobjs_off.']';
	...
	/**
	 * Return an hash code used to ensure that the serialized data has been generated by this pdf instance.
	...
	/**
	 * Serialize an array of parameters to be used with pdf tag in HTML code.
	...
	/**
	 * Unserialize parameters to be used with pdf tag in HTML code.
	 * @param string $data serialized data
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
		...
		switch($tag['value']) {
			...
			case 'tcpdf': {
				if (defined('K_TCPDF_CALLS_IN_HTML') AND (K_TCPDF_CALLS_IN_HTML === true)) {
					// Special tag used to call pdf methods
					if (isset($tag['attribute']['method'])) {
	...
	/**
	 * Stores a copy of the current pdf object used for undo operation.
	 * @public
	 * @since 4.5.029 (2009-03-19)
	 */
	...
	/**
	 * Delete the copy of the current pdf object used for undo operation.
	 * @public
	 * @since 4.5.029 (2009-03-19)
	 */
	...
	/**
	 * This method allows to undo the latest transaction by returning the latest saved pdf object with startTransaction().
