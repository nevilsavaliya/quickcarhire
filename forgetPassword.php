...
if (isset($_POST['submit'])) {
    ...
    $emailQuery = mysqli_query($conn, "SELECT email FROM Customer WHERE Email = '$email'");

    if (mysqli_num_rows($emailQuery) < 1)
        echo "<script>alert('Email does not exists');window.location.replace('./Index.php');</script>";
    else {
